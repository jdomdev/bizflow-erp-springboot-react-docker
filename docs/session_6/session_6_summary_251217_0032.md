...
- Se añadieron dos utilidades en shell:
  - `scripts/backup_prod_db.sh` apunta a `erp-prod-db-container` y vuelca `erp_prod_db` en `backups/erp_db_backup_<timestamp>.dump`.
  - `scripts/backup_dev_db.sh` apunta a `erp-dev-db-container` y vuelca `erp_dev_db` en `backups/dev/erp_dev_db_backup_<timestamp>.dump`.
...
- Se dieron permisos de ejecución y se hicieron pruebas manuales, generando:
  - Dump de producción en `backups/erp_db_backup_20251216_235915.dump`.

...
Los scripts de copias de seguridad ([scripts/backups/backup_dev_db.sh](../../scripts/backups/backup_dev_db.sh) y [scripts/backups/backup_prod_db.sh](../../scripts/backups/backup_prod_db.sh)) evitan de forma deliberada interpretar los `.env.*`. Solo leen `DB_PASSWORD` del shell que los invoca para garantizar que:
...
./scripts/backups/backup_dev_db.sh
```
...
3. **Inyección puntual en un proceso**
   ```bash
   DB_PASSWORD="supersecret" ./scripts/backups/backup_dev_db.sh
   ```
...
4. **Revisar `.gitignore`**: confirma que los scripts auxiliares con credenciales embebidas (p. ej. `init_expense_data.sh`) siguen ignorados o se han adaptado para leer de variables de entorno como hacen los scripts de backup.
